...
const WELCOME = "Hey there, I'm Mahir. Feel free to ask me anything about my work or background — happy to chat.";
const PLACEHOLDERS = 'Ask me anything...';
const FALLBACKS = "Hmm, not sure what to say to that one — ask me about my work, experience, or anything design-related.";
const SPEECH_LANG_CODES = 'en-US';
    ...
    ensureVoices(function(){
      setTimeout(function(){
        try {
          ...
          u.lang='en-US';u.rate=0.92;u.pitch=0.95;u.volume=1.0;
          try {
            const vs=synth.getVoices();
            var femaleSuffix=/female|samantha|karen|moira|tessa|veena|sara|fiona|catherine|helen|siri|yelda|milena/i;
            var voice=vs.find(function(v){
              if(!v.lang.startsWith('en')) return;
              if(femaleSuffix.test(v.name)) return;
              return /male|david|mark|daniel|arthur|alex|james|john|paul|mike|steve|peter|tom|tim|henry|google/i.test(v.name);
            });
            ...
            if(!voice) voice=vs.find(x=>x.lang.startsWith('en'||'en'));
...
function buildRecog(){
  ...
  r.lang='en-US'; r.continuous=isAlways; r.interimResults=isAlways; r.maxAlternatives=1;
  r.onstart=function(){
    listening=true; micBtn.classList.add('on');
    lbl.textContent=isAlways?'Always listening (EN)...':'Listening (EN)...'; lbl.classList.add('on');
  };
  ...
  function doStart(){
    if(recog){
      try{ recog.start(); listening=true; micBtn.classList.add('on');
        lbl.textContent=alwaysListen?'Always listening (EN)...':'Listening (EN)...'; lbl.classList.add('on'); return;
